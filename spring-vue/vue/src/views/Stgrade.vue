...
<!--        <el-button type="success" icon="el-icon-top">导出</el-button>-->
        <el-upload
                action="http://localhost:9090/stgrade/import"
    ...
    export default {
        ...
        methods:{
            deleteBatch(){
                ...
                request.post("/api/stgrade/deleteBatch",this.ids).then(res=>{
                    if (res.code==='0'){
            ...
            save(){
                if (this.form.id){//判断是否是更新
                    request.put("/api/stgrade",this.form).then(res=>{
                        console.log(res)
                    ...
                    request.post("/api/stgrade", this.form).then(res => {
                        console.log(res)
                        this.$message({
                            type:"success",
                            message:"新增成功"
            ...
            handleDelete(id){//删除
                console.log(id)
                request.delete("/api/stgrade/"+id).then(res=>{
                    if (res.code=='0'){//判断返回结果是否成功
                        this.$message({
                            type:"success",
                            message:"删除成功"
            ...
            exportUser() {
                ...
                location.href = "http://localhost:9090/stgrade/export";
            },
        }
    }
</script>
